...
const elScoreEsquerda = document.getElementById("scoreEsquerda");
const elScoreDireita = document.getElementById("scoreDireita");
...
const velocidadeBarra = 0.01;
const limiteBarra = 0.8; 
...
function atualizaPlacar(){
    elScoreEsquerda.textContent = scoreEsquerda;
    elScoreDireita.textContent = scoreDireita;
}
...
function resetBola(){
    ...
    // Direção aleatória para variar o início da jogada
    txBola_offset = (Math.random() < 0.5 ? -1 : 1) * 0.005;
...
function verificaColisaoBarras(){
    ...
    if (
        txBola_offset < 0 &&
        txBola - raioBola <= -0.9 + metadeLarguraBarra &&
        txBola > -0.9 &&
        Math.abs(tyBola - tyBE) <= metadeAlturaBarra + raioBola
    ) {
        txBola_offset = -txBola_offset;
        txBola = -0.9 + metadeLarguraBarra + raioBola;
    }
    ...
    if (
        ...
        txBola + raioBola >= 0.9 - metadeLarguraBarra &&
        txBola < 0.9 &&
        Math.abs(tyBola - tyBD) <= metadeAlturaBarra + raioBola
    ) {
        txBola_offset = -txBola_offset;
        txBola = 0.9 - metadeLarguraBarra - raioBola;
    }
